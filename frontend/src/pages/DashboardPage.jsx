...
function PatientOverview() {
  ...
  const [reschedule,setReschedule]=useState(null),[availableSlots,setAvailableSlots]=useState([]),[slotsLoading,setSlotsLoading]=useState(false),[selectedSlot,setSelectedSlot]=useState('');
  ...
  async function loadRescheduleSlots(item,date){setReschedule({...item,newDate:date});setSelectedSlot('');setAvailableSlots([]);if(!date)return;setSlotsLoading(true);setAppointmentsError('');try{const data=await apiRequest(`/appointments/availability/reschedule?doctorName=${encodeURIComponent(item.doctorName)}&date=${encodeURIComponent(date)}&excludeBookingId=${encodeURIComponent(item.bookingId)}`);setAvailableSlots(data.availableSlots||[])}catch(error){setAppointmentsError(error.message)}finally{setSlotsLoading(false)}}
  function rescheduleVisit(item){setReschedule({...item,newDate:item.date});loadRescheduleSlots(item,item.date)}
  async function confirmReschedule(){if(!reschedule?.newDate||!selectedSlot)return;setSlotsLoading(true);setAppointmentsError('');try{const result=await apiRequest(`/appointments/${encodeURIComponent(reschedule.bookingId)}/reschedule`,{method:'PATCH',body:JSON.stringify({date:reschedule.newDate,time:selectedSlot,doctorName:reschedule.doctorName})});setSyncMessage(result.message||'Appointment rescheduled. Patient and doctor calendars are synchronizing.');setReschedule(null);setSelectedSlot('');await synchronize()}catch(error){setAppointmentsError(error.message);if(error.data?.availableSlots)setAvailableSlots(error.data.availableSlots)}finally{setSlotsLoading(false)}}
  const rescheduleDialog=reschedule&&<div className="reschedule-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setReschedule(null)}}><section className="reschedule-dialog" role="dialog" aria-modal="true" aria-labelledby="reschedule-title"><div className="reschedule-dialog__head"><div><small>Appointment {reschedule.bookingId}</small><h2 id="reschedule-title">Choose a new open slot</h2><p>{reschedule.doctorName}</p></div><button type="button" onClick={()=>setReschedule(null)} aria-label="Close rescheduling"><X/></button></div><label>New appointment date<input type="date" min={new Date().toLocaleDateString('en-CA')} value={reschedule.newDate||''} onChange={event=>loadRescheduleSlots(reschedule,event.target.value)}/></label><div className="available-slot-section"><b>Available clinic times</b>{slotsLoading?<div className="slot-loading"><RefreshCw className="spinning"/>Checking live availability…</div>:availableSlots.length?<div className="available-slot-grid">{availableSlots.map(slot=><button type="button" className={selectedSlot===slot?'selected':''} key={slot} onClick={()=>setSelectedSlot(slot)}>{slot}</button>)}</div>:<p>No open slots on this date. Choose another weekday.</p>}</div><div className="reschedule-dialog__actions"><button type="button" onClick={()=>setReschedule(null)}>Keep current appointment</button><button type="button" disabled={!selectedSlot||slotsLoading} onClick={confirmReschedule}>Confirm reschedule</button></div></section></div>;
  if (appointments) {

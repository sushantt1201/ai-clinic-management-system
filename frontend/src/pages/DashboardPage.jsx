...
function PatientOverview() {
  const [appointments,setAppointments]=useState([]), [appointmentsError,setAppointmentsError]=useState('');
  useEffect(()=>{let active=true;apiRequest('/appointments').then(data=>{if(active)setAppointments(data.appointments||[])}).catch(error=>{if(active)setAppointmentsError(error.message)});return()=>{active=false}},[]);
